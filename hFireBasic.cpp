...
hFireBasic::hFireBasic() {
    ...
    if (!hTexture.loadFromFile("resources/Textures/FuegoSpriteSheet.png")) {

        //comentario error
        exit(0);
    }
    animationInicio = new Animation();
    animationInicio->setSpriteSheet(hTexture);
...
void hFireBasic::cast(sf::Vector2f posicion, sf::RenderWindow *mWindow) {
    ...
    SetPosition(posicion);
        ...
        float angleShot = atan2(mousePosition.y - GetPosition().y,
...
void hFireBasic::DrawWithInterpolation(sf::RenderWindow& window, float interpolation){
	Draw(window, GetPreviousPosition(), GetPosition() , interpolation);
}
